#!/usr/bin/env bash
# Stage the Broker Odin collection and pack thirp-broker-<VERSION>.tar.gz.
# Embed SDK packages plus auth and broker. No C ABI, CLIs, or tests.
...
STAGE="${ROOT}/dist/thirp-runtime-broker-${VERSION}"
BROKER_NAME="thirp-runtime-broker-${VERSION}"

if [[ ! -f "${OUT}/thirp-runtime-${VERSION}.spdx.json" ]]; then
	echo "release_broker: missing ${OUT}/thirp-runtime-${VERSION}.spdx.json" >&2
	exit 1
fi
if [[ ! -f "${OUT}/PROVENANCE.txt" ]]; then
	echo "release_broker: missing ${OUT}/PROVENANCE.txt" >&2
	exit 1
fi
if [[ ! -f "${ROOT}/docs/SDK.md" ]]; then
	echo "release_broker: missing docs/SDK.md" >&2
	exit 1
fi
...
echo "release_broker: walking Broker import closure (plus agent/caller)"
mapfile -t PACKAGES < <(broker_walk_packages "$ROOT")
...
broker_write_manifest() {
	local manifest="${STAGE}/BROKER_MANIFEST.json"
	local list rel size hash first

	list="$(mktemp)"
	(
		cd "$STAGE"
		find . -type f ! -name BROKER_MANIFEST.json ! -name SHA256SUMS | sed 's|^\./||' | sort
	) >"$list"

	{
		...
		echo "  \"public_packages\": $(sdk_json_string_array $BROKER_PUBLIC_PACKAGES),"
		echo "  \"support_packages\": $(sdk_json_string_array $BROKER_SUPPORT_PACKAGES),"
		echo '  "files": ['
		first=1
...
TAR="${OUT}/${BROKER_NAME}.tar.gz"
rm -f "$TAR"
tar --sort=name --mtime="$DATE" --owner=0 --group=0 --numeric-owner \
	-C "${ROOT}/dist" -czf "$TAR" "$BROKER_NAME"

echo "release_broker: wrote ${TAR}"
