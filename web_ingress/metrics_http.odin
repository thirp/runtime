...
ingress_metrics_listen :: proc(server: ^IngressServer, endpoint: net.Endpoint) -> trans.TransportError {
	if server == nil {
		return .Closed
	}
...
ingress_metrics_start :: proc(server: ^IngressServer) {
	if server == nil || !server.metrics_listening {
		return
	}
	server.metrics_thread = thread.create_and_start_with_poly_data(server, metrics_http_serve)
}

ingress_metrics_stop :: proc(server: ^IngressServer) {
	if server == nil {
		return
	}
...
ingress_metrics_endpoint :: proc(server: ^IngressServer) -> (net.Endpoint, trans.TransportError) {
	if server == nil || !server.metrics_listening {
		return {}, .Closed
	}
...
metrics_http_serve :: proc(server: ^IngressServer) {
	for {
...
metrics_http_handle :: proc(server: ^IngressServer, conn: ^trans.Connection) {
	...
	for n < len(req) {
		...
		n += got
		if metrics_has_header_end(req[:n]) {
			break
		}
	}
	path, ok := metrics_http_request_path(req[:n])
	if !ok {
...
metrics_has_header_end :: proc(buf: []u8) -> bool {
	return strings.contains(string(buf), "\r\n\r\n")
}

metrics_http_request_path :: proc(buf: []u8) -> (string, bool) {
	text := string(buf)
